...
from examples.hyperparameteropt.loaders import make_loaders
...
class Trainer:
	...
	def update_args(self):
		self.args.training.lower.model.n_features =  self.meta_data['n_features']
		self.args.training.lower.model.n_classes = self.meta_data['n_classes']
		self.args.training.upper.model.dim = self.meta_data['n_features']
	...
	def train(self):
		...
		while self.counter<=self.count_max:
			...
			#self.logger.log_artifacts({'weights':weights}, f"weights/{self.epoch}", artifact_type="pickle")
